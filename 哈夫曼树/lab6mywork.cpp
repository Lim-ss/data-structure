...
void printHtree1(HTNode ht[])
{
	...
	for (int i = 0; i < m; ++i)
		if (i < n)
		{
			printf("%2x %3d -> %5d => p:%3d lc:%3d rc:%3d\n", ht[i].id, ht[i].id, ht[i].w, ht[i].parent==NULL?-1:ht[i].parent->id, ht[i].lchild==NULL?-1:ht[i].lchild->id, ht[i].rchild==NULL?-1:ht[i].rchild->id);
			printf("打印\n");
		}
		else
			printf("%2x %3d -> %5d => p:%3d lc:%3d rc:%3d\n", ht[i].id, ht[i].id+256, ht[i].w, ht[i].parent==NULL?-1:ht[i].parent->id, ht[i].lchild==NULL?-1:ht[i].lchild->id, ht[i].rchild==NULL?-1:ht[i].rchild->id);
}
...
void getCode(HTNode ht[], int c, unsigned *code1, int *clen1)
{
	...
	parent = ht[c].parent->id;
	while (parent >= 0)
	{
		code = code >> 1;
		if(ht[parent].lchild == NULL) code |= mask;
		else ht[parent].lchild->id == c ? code : code |= mask; // 如果是右孩子，那么最高位设置为1，否则就是0
		clen++;
		c = parent;
		//parent = ht[c].parent;
		if(ht[c].parent == NULL)
...
int main()
{
	...
	printf("test5\n");
	/*************************** 以下为解码内容 ****************************/
	char ofile[256] = "lab6_uzip";
	unzip(zfile, ofile);
	char file1[]="lab6/test";
	char file2[]="lab6_uzip";
